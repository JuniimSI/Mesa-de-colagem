...
export default function Thumbs({ fotos, onRemove, max = 12 }) {
  const mostrar = fotos.slice(0, max)
  const sobra = fotos.length - mostrar.length
  return (
    <div className="tiras">
      {mostrar.map((f, i) => (
        <div className="thumb-wrap" key={i}>
          <Thumb foto={f} />
          {onRemove && (
            <button className="thumb-x" onClick={() => onRemove(i)} aria-label="Remover foto">×</button>
          )}
        </div>
      ))}
